function hout = plot_puma(x, y, z, phi, theta, psi, theta1, theta2, theta3, theta4, theta5, theta6, redValue, greenValue, blueValue, hin)
%
...
% Pennsylvania. It plots many things, as explained below.
% 
% A gray coordinate frame at the position and orientation specified by
% the first six inputs.  x, y, and z specify the desired position of
% the origin of the sixth frame with respect to frame zero.  phi,
% theta, and psi are ZYZ Euler angles that specify the desired
% orientation of the sixth frame with respect to frame zero.
%
% The PUMA robot in the configuration specified by the six joint angle
% inputs, which are the seventh through twelfth arguments, named
% theta1 through theta6.
%
% A point of light in the specified RGB color at the end-effector tip.
% Each color value should range from 0 to 1.  Set the color to all
% zeros (black) to plot no point of light for this pose.
%
% This function returns a vector of handles to the items it has
% plotted so that this function can be used to update a plot rather
% than replace it.
%
% The final input argument, which is optional, is a vector of handles
% from a previous call to this function.  When provided, this function
% updates the plot rather than replacing it.
...
if (sum(isnan(thetas)) > 0)
    ...
    % requested.  Put the robot in the home configuration and turn it
    % red.
    theta1 = 0;
    theta2 = 0;
    theta3 = 0;
...
[points_to_plot, x60, y60, z60] = puma_fk(theta1, theta2, theta3, theta4, theta5, theta6);
...
% Calculate the coordinates of line segments showing the x, y, and z
% unit vectors of the desired location for frame 6 in frame 0. Each of
% these vectors starts at the desired origin and ends at the distance
% vlen along the designated axis.  We calculate the location of the
% end by multiplying Hdes into a scaled unit vector in the correct
% direction.
...
if (nargin == 16)
    ...
    % The user has passed in an array of handles so the plot can be
    % updated instead of replaced.  Split them out.
    hrobot = hin(1);
    hx60 = hin(2);
    hy60 = hin(3);
    hz60 = hin(4);
    ...
    % Plot the robot in 3D using big dots at the points and thick
    % lines to connect neighboring points.  Keep a handle to the plot.
